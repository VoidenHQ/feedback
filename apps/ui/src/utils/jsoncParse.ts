...
type JSONPrimitiveValue = JSONNullValue | JSONEOFValue | JSONStringValue | JSONNumberValue | JSONBooleanValue;
...
export default function jsonParse(str: string): JSONObjectValue | JSONArrayValue {
  string = str;
  strLen = str.length;
  start = end = lastEnd = -1;
...
function parseObj(): JSONObjectValue {
  ...
  return {
    ...
    members,
    comments: comments.length > 0 || trailingComments.length > 0 ? [...comments, ...trailingComments] : undefined,
  };
}
...
function parseArr(): JSONArrayValue {
  ...
  while (!skip("]")) {
    ...
    if (pendingComments.length > 0 && typeof value === "object") {
      (value as JSONObjectValue).comments = [...((value as JSONObjectValue).comments || []), ...pendingComments];
      pendingComments = [];
    }
    values.push(value);
  }
...
function lex() {
  ...
  while (true) {
    ...
    if (code === 47 && string.charCodeAt(end + 1) === 42) {
      ...
      let commentText = "";
      while (code !== 0 && !(code === 42 && string.charCodeAt(end + 1) === 47)) {
        commentText += String.fromCharCode(code);
        ch();
      }
