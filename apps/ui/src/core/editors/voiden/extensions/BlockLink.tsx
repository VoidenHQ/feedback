...
const LinkedBlockNodeView = ({ node, editor, getPos }: any) => {
  ...
  return (
    <NodeViewWrapper className="my-2">
      {/* Header bar with "IMPORTED" label and clickable filename - always visible */}
      <div className="px-2 py-1.5 flex items-center justify-between border-b border-border/10 bg-accent/5">
        <div className="flex items-center gap-2 flex-1">
          <Link2 size={12} className="text-text opacity-60" />
          <span className="font-mono text-xs text-text opacity-80">IMPORTED:</span>
          <button
            onClick={handleGoToOriginal}
            className="font-mono text-xs text-accent hover:text-text transition-colors hover:underline"
