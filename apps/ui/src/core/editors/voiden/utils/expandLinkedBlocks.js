 ...
 */
export async function expandLinkedBlocks(json, depth = 0) {
    ...
    if (json.content && Array.isArray(json.content)) {
        const expandedContent = await Promise.all(json.content.map(child => expandLinkedBlocks(child, depth + 1)));
        return {
            ...json,
 ...
 */
export async function expandLinkedBlocksInDoc(doc) {
    if (!doc.content || !Array.isArray(doc.content)) {
        return doc;
    }
    const startTime = performance.now();
    const expandedContent = await Promise.all(doc.content.map(node => expandLinkedBlocks(node)));
