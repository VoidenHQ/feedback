...
export function getBuildDefine(env: ConfigEnv<"build">) {
  ...
  // Inject S3 configuration - bucket names are hardcoded
  const s3Region = process.env.S3_REGION || "eu-west-1";

  define["__S3_BUCKET_NAME_BETA__"] = JSON.stringify("voiden-beta-releases");
  define["__S3_BUCKET_NAME_STABLE__"] = JSON.stringify("voiden-releases");
  define["__S3_REGION__"] = JSON.stringify(s3Region);
