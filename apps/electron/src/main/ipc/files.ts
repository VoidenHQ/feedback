...
export function registerFileIpcHandlers() {
  ...
  ipcMain.handle("files:write", async (_event, filePath, content, tabId): Promise<string | null> => {
  ...
  ipcMain.handle("files:create-void", async (_event, projectName: string, fileName: string) => {
    const result = await createVoidFile(projectName, fileName);
    return result;
  });

  ipcMain.handle("files:create", async (_event, projectName: string, fileName: string) => {
    const result = await createFile(projectName, fileName);
    return result;
  });

  ipcMain.handle("files:createDirectory", async (_event, path: string, dirName?: string) => {
    return await createDirectory(path, dirName);
  });

  ipcMain.handle("files:getDirectoryExist", async (_event, path: string, dirName?: string) => {
    return await getDirectoryExist(path, dirName);
  });

  ipcMain.handle("files:getFileExist", async (_event, path: string, fileName?: string) => {
    return await getFileExist(path, fileName);
  });

  ipcMain.handle("files:create-new-project", async (_event, dirName?: string) => {
  ...
  ipcMain.handle("files:getFiles", async (_event, filePaths: string[], isExternal?: boolean) => {
    const files = await Promise.all(
      filePaths.map(async (filePath) => {
        ...
        try {
          ...
          return { fullPath, fileName: path.basename(fullPath), mimeType: null, data: null, error: error.message };
        }
      }),
    );
    return files;
  ...
  ipcMain.handle("files:rename", async (_event, oldPath: string, newName: string) => {
    return await renameFileOrDirectory(oldPath, newName, getActiveStates, saveActiveStates);
  });
