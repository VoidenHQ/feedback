...
export const getNodeType = (editor: Editor) => {
  ...
  if (isTriggeredFromTable) {
    ...
    if (["headers-table", "query-table", "url-table", "multipart-table", "path-table"].includes(type)) return type;
    return "table";
  }
...
export const getAllowedSuggestionPopup = (suggestionType: string) => (props: { editor: Editor; state: EditorState; range: Range }) => {
  ...
  const isCustomNode = ["method", "url", "headers-table", "query-table", "url-table", "table"].includes(nodeType);
