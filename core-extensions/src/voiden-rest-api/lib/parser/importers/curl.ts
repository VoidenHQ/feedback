...
import { getFormDataBody } from "./preproc";
...
const importCommand = (parseEntries: ParseEntry[], rawData?: string): ImportRequest => {
  parseEntries = parseEntries
    .filter((i) => {
      return typeof i === "string" ? i.trim().length > 0 : true;
    })
  ...
  const findHeader = (headerKey: string) => {
    const header = pairsByName["H"] || pairsByName["header"];
    const temp = header?.find((h) => (h as string).includes(headerKey));
    if (!temp) return null;
    const [name, value] = (temp as string).split(/:(.*)$/);
  ...
  for (let i = 1; i < parseEntries.length; i++) {
    ...
    if (typeof parseEntry === "string" && parseEntry.match(/^-{1,2}[\w-]+/)) {
      ...
      if (isSingleDash && name.length > 1) {
        ...
        value = nextEntry;
        i++; // Skip next one
      } else if (parseEntry.includes("data-raw") && contentType?.value?.includes("multipart/form-data")) {
        value = nextEntry ? (rawData ? getFormDataBody(rawData, contentType?.value) || "" : "") : "";
        i++; // Skip next one
      } else {
        value = true;
      }
  ...
  const formDataParams = [...((pairsByName.form as string[] | undefined) || []), ...((pairsByName.F as string[] | undefined) || [])].map((str) => {
 ...
 */
const dataFlags = CURL_DATA_FLAGS;
...
const pairToParameters = (pair: Pair, allowFiles = false): Parameter[] => {
  ...
  return pair.split("&").map((pair) => {
    if (pair.startsWith("@") && allowFiles) {
      const [name, fileName] = pair.split("@");
      return { name, fileName, type: "file" };
    }
    try {
      const isJson = JSON.parse(pair);
      if (isJson) return { name: "", value: pair };
    } catch (error) {
      // do nothing
    }
...
export const convert: Converter = (rawData) => {
  ...
  const unProcessedRawData = rawData.match(/--data-raw\s+\$*'(.*?)'/);

  if (!rawData.match(/^\s*curl /)) {
    return null;
  }

  // Convert Windows CMD-style curl to bash-style
  ...
  // - Multiple ^^ patterns (escaped carets)
  const isWindowsCmdCurl = /\^["'\n\r]|\^\^/.test(rawData);

  if (isWindowsCmdCurl) {
    // First, handle line continuations - remove ^ at end of lines
    processedData = processedData.replace(/\s*\^\s*\n\s*/g, ' ');
    processedData = processedData.replace(/\s*\^\s*$/gm, ' ');

    // Windows CMD escaping rules:
    // ^^ -> ^ (escaped caret becomes literal caret)
    // ^" -> " (escaped quote becomes quote)
    // ^{ -> { (escaped brace becomes brace)
    // ^^^^ -> ^^ (two escaped carets become two literal carets)
    // etc.

    // Replace ^ followed by any character with just that character
    // We only do this ONCE because ^^ should become ^, not disappear entirely
    processedData = processedData.replace(/\^(.)/g, '$1');
  }

  // Parse the whole thing into one big tokenized list
  const parseEntries = parse(processedData.replace(/\\n/g, " "), undefined, undefined);
  ...
  const requests: ImportRequest[] = commands
    ...
    .map((v) => importCommand(v, (unProcessedRawData && unProcessedRawData[1]) || ""));

  return requests;
};

export const convertToCurl = (requestObject: Request): string | null => {
  if (!requestObject.method || !requestObject.url) {
    return null;
  }

  let curlCommand = `curl -X ${requestObject.method} "${requestObject.url}"`;

  if (requestObject.headers && requestObject.headers.length > 0) {
    requestObject.headers.forEach((header: RequestParam) => {
      curlCommand += ` -H "${header?.key}: ${header?.value}" `;
    });
  }

  if (requestObject.body) {
    curlCommand += ` --data '${requestObject.body}'`;
  }
