...
export const createResponseBodyNode = (
  ...
  const ResponseBodyComponent = ({ node }: any) => {
    ...
    const renderRaw = () => {
      ...
      // Allow more vertical space to reduce scrolling friction
      const lines = displayValue.split('\n').length;
      const lineHeight = 20; // Approximate line height in pixels

      // Calculate height: fit content up to 80vh or 1200px
      const contentHeight = lines * lineHeight + 60;
      const viewportMaxHeight = window.innerHeight * 0.8;
      const maxHeight = Math.min(contentHeight, viewportMaxHeight, 1200);

      // If content fits, use exact height; otherwise use maxHeight and allow scrolling
        ...
        <div style={{ height: 'auto', overflow: 'visible' }}>
          <style>{`
            .response-body-editor .cm-editor {
              height: ${shouldFit ? `${contentHeight}px` : `${maxHeight}px`} !important;
    ...
    return (
      <NodeViewWrapper className="response-body-node" style={{ userSelect: 'text' }}>
        <div className="my-2">
          ...
          <div
            className="flex items-center justify-between bg-bg border-b !border-solid !border-[rgba(0,0,0,0.2)] px-2 py-1.5 header-bar"
          ...
          >
            <div className="flex items-center gap-2 flex-1" style={{ userSelect: 'none' }}>
              ...
              {contentType && (
                <span className="text-xs text-comment font-mono" style={{ pointerEvents: 'none' }}>{contentType}</span>
              )}
            </div>
                ...
                <>
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded ${
                      viewMode === "preview"
                        ? "bg-active text-text"
                        : "text-comment hover:bg-active/50"
                    }`}
